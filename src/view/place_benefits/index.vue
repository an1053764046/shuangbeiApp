...
<div>
    ...
    <div class="timeStr">
      ...
      <div>
        ...
        <van-button plain type="primary" size="small" @click="getplace({placeid,start:starTime,end:endTime})">查询</van-button>
    </div>  
		<van-list
      v-model="loading"
      :finished="finished"
      ...
      this.show1 = false;
		...
		getplace(data){
			getplaceList(data).then(res=>{
				...
				this.loading=false
				this.finished=true
				this.total='总共'+res.data.total+'台设备'
			})
		},
	},
	created(){
		...
		this.getplace({placeid:this.placeid,start,end})
	}
};
</script>
